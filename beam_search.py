...
class BeamSearch(object):
  ...
  def __init__(self, model, beam_size, start_token, end_token, max_steps):
    ...
    Args:
      ...
      max_steps: int, upper limit on the size of the hypothesis : hps.dectimesteps (not 1)
    """
    self._model = model
  ...
  def BeamSearch(self, sess, enc_inputs, enc_seqlen_doc, enc_seqlen_sent, enc_weights_sent, enc_weights_doc):
    """Performs beam search for decoding.
    Args:
      sess: tf.Session, session
      enc_inputs: ndarray of shape (enc_length, 1) (It is wrong! ), the document ids to encode
      enc_seqlen_doc: ndarray of shape (1), the length of the sequnce
      enc_seqlen_sent
    Returns:
    ...
    #enc_top_states: [batch_size, enc_sent_num, 2num_hidden]
    #dec_in_state: [1, fw_h_state_size]
    enc_top_states, enc_top_states_sent, dec_in_state = self._model.encode_top_state(
        sess, enc_inputs, enc_seqlen_doc, enc_seqlen_sent, enc_weights_sent)
    ...
    while steps < self._max_steps and len(results) < self._beam_size:
      ...
      topk_ids, topk_log_probs, new_states = self._model.decode_topk(
          sess, latest_tokens, enc_top_states, enc_top_states_sent, states, enc_weights_doc, enc_weights_sent)
      ...
      for i in xrange(num_beam_source):
        ...
        for j in xrange(self._beam_size):
          ...
          if(len(h_tokens) < 3):
            ...
            m = 0
            for k in xrange(len(h_tokens)-2):
              if(h_tokens[k]==h_tokens[-2] and h_tokens[k+1]==h_tokens[-1] and h_tokens[k+2]==topk_ids[i, j]):
                m=1
                break
            if(m == 0 and len(all_hyps)<2*self._beam_size):
              all_hyps.append(h.Extend(topk_ids[i, j], topk_log_probs[i, j], ns))
